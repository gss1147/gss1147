...
from Automated_Memory_Core import get_memory_manager
...
class SystemOrchestrator:
    ...
    def _initialize_modules(self) -> None:
        ...
        self.modules = {
            ...
            "memory": get_memory_manager(self.work_dir),
    ...
    def gui_action(self, module_key: str, action: str, payload: Dict[str, Any]):
        ...
        Returns a small dict so the GUI can display a concise result.
        """
        try:
            ...
            # Universal runner
            if action == "run_text":
                if module_key in {"recursive", "neural_symbolic", "reasoners", "hybrid", "info_core"}:
                    if not text:
                        text = "GUI default test input"
                    if module_key == "recursive":
                        out = self.modules["recursive"].process_input(text)
                    elif module_key == "neural_symbolic":
                        # Seed a couple rules to make it non-trivial.
                        if getattr(self.modules["neural_symbolic"], "rules", []) == []:
                            self.modules["neural_symbolic"].add_rule("IF recursive THEN reflect")
                            self.modules["neural_symbolic"].add_rule("IF file THEN retrieve")
                        out = self.modules["neural_symbolic"].process_logical_query(text)
                    elif module_key == "reasoners":
                        out = self.modules["reasoners"].solve_problem(text)
                    elif module_key == "hybrid":
                        out = self.modules["hybrid"].process_input(text)
                    else:  # info_core
                        out = self.modules["info_core"].enhance_input(text)
                    return out if isinstance(out, dict) else {"status": "success", "type": "run_text"}
                if module_key == "memory":
                    if not text:
                        text = "GUI memory note"
                    return self.modules["memory"].add_stm(text, meta={"source": "GUI"})
                return {"status": "error", "type": "gui_action", "message": f"Unknown module_key={module_key}"}

            # Recursive
            if module_key == "recursive" and action == "test_recursion":
                depth = int(payload.get("max_depth", 32))
                out = self.modules["recursive"].process_input(text or "GUI recursion test", max_depth=depth)
                return out

            # Neural symbolic
            if module_key == "neural_symbolic" and action == "run_symbolic_query":
                engine = str(payload.get("engine", "deductive"))
                q = text or f"symbolic_test engine={engine}"
                out = self.modules["neural_symbolic"].process_logical_query(q)
                return {**out, "engine": engine}

            # Agentic
            if module_key == "agentic" and action == "propose_patch":
                path = self.modules["agentic"].propose_patch(
                    title="gui_proposal",
                    proposal=(
                        "GUI requested a safe patch proposal.\n"
                        "Populate this file with a unified diff and a brief rationale.\n"
                        "No auto-apply occurs in this system."
                    ),
                    metadata={"source": "GUI"},
                )
                return {"status": "success", "type": "patch_proposal", "path": path}

            if module_key == "agentic" and action == "debug_agents":
                return self.modules["agentic"].debug()

            if module_key == "agentic" and action == "evolve_routing":
                return self.modules["agentic"].evolve_routing()

            # Reasoners
            if module_key == "reasoners" and action == "solve_problem":
                mode = str(payload.get("mode", "hybrid"))
                q = text or ("2+2" if mode == "commonsense" else "(7*9)+3")
                out = self.modules["reasoners"].solve_problem(q)
                return {**out, "mode": mode}

            # Hybrid
            if module_key == "hybrid" and action == "meta_train":
                return self.modules["hybrid"].meta_train_step()

            if module_key == "hybrid" and action == "rl_step":
                return self.modules["hybrid"].rl_step()

            if module_key == "hybrid" and action == "consolidate_memory":
                return self.modules["memory"].consolidate(limit=int(payload.get("limit", 25)))

            # Memory
            if module_key == "memory" and action == "view_stm":
                return self.modules["memory"].query_stm(limit=int(payload.get("limit", 20)))

            if module_key == "memory" and action == "view_ltm":
                return self.modules["memory"].query_ltm(limit=int(payload.get("limit", 20)))

            if module_key == "memory" and action == "clear_memory":
                return self.modules["memory"].clear(stm=True, ltm=bool(payload.get("ltm", False)))

            # Information core
            if module_key == "info_core" and action == "self_test":
                ft = self.modules["info_core"].fine_tune_once() if hasattr(self.modules["info_core"], "fine_tune_once") else {"status": "n/a"}
                stats = self.modules["info_core"].get_stats()
                return {"status": "success", "type": "info_core_self_test", "fine_tune": ft, "stats": stats}

            if module_key == "info_core" and action == "process_path":
                target = str(payload.get("path", "")).strip()
                if not target:
                    return {"status": "error", "type": "info_core", "message": "No path provided"}
                res = self.modules["info_core"].process_file(target)
                return {"status": res.get("status", "success"), "type": "info_core_process", "path": target}

            return {"status": "error", "type": "gui_action", "message": f"Unhandled action: {module_key}.{action}"}
        except Exception as e:
            return {"status": "error", "type": "gui_action", "message": str(e)}
    ...
    def process_user_input(self, input_text: str) -> Dict[str, Any]:
        ...
        try:
            # Persist to STM (safe, bounded) early
            try:
                self.modules["memory"].add_stm(
                    str(input_text),
                    meta={"source": "user_input", "t": datetime.now().isoformat()},
                )
            except Exception:
                pass
            ...
            # Store a compact response summary for recall
            try:
                summary = {
                    "confidence": conf,
                    "reasoner_type": reasoning_result.get("type") if isinstance(reasoning_result, dict) else None,
                    "hybrid_type": hybrid_result.get("type") if isinstance(hybrid_result, dict) else None,
                }
                self.modules["memory"].add_stm(
                    f"AI_RESULT: {json.dumps(summary, ensure_ascii=False)}",
                    meta={"source": "system", "t": datetime.now().isoformat()},
                )
            except Exception:
                pass
